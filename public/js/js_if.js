...
// === FONTE DE DADOS MOCK (ATIVA AGORA) ===
function dadosMocados() {
    ...
    const netArr = mk(30, 90, 60, 5, 200); // Mbps
...
// Ative o mock gerador global
const mockGen = dadosMocados();

// Exemplo de loop de "tempo real" a cada ~2s
let timerMock = null;
...
/*
// Polling no back a cada 3s — DESCOMENTAR QUANDO FOR LIGAR NO BACKEND
let timerDB = null;
...
class GerenciadorInterface {
    ...
    calcularStatusSistema() {
        ...
        const valores = [
            ...
            ultimoDado.memoria,
            ultimoDado.disco,
            (ultimoDado.rede / 200) * 100 // Normalizar rede para %
        ];
...
function inicializarDados() {
    ...
    // Adicionar alguns pontos iniciais para tempo real
    for (let i = 0; i < 10; i++) {
        const ponto = mockGen.proximoPonto('INFRA-EDGE-01');
        dadosTempoReal.push(ponto);
    }
...
function atualizarGraficoHistorico() {
    ...
    const dados = estadoApp.dadosCompletos.slice(-100); // Últimos 100 pontos
...
function atualizarTabelaHistorico() {
    ...
    // Simular dados históricos agregados
    const dadosAgregados = [];
    const agora = new Date();
...
function configurarNavegacao() {
    ...
    document.querySelectorAll('.nav-item').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Liga/desliga mock conforme necessidade
    ...
    if (edgeSelector) {
        edgeSelector.addEventListener('change', () => {
            // Limpar dados atuais e gerar novos
            dadosTempoReal = [];
            gerenciadorInterface.atualizarInformacoesSistema();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!sistemaLogin.verificarSessao()) {
        ...
        // Se já está logado, inicializar tudo
        setTimeout(() => {
            inicializarDados();
            inicializarGraficos();
            configurarNavegacao();
